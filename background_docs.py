...
WB_INSTRUMENT_GUIDE = {
    "IPF": {
        ...
        "fcv_levers": (
            ...
            "FM, deferred fiduciary assessments, faster procurement in acute crisis settings); "
            "OP 7.30 (De Facto Governments — governs engagement where government changed through "
            "unconstitutional means); "
            "OP 7.60 (Disputed Areas — for projects in areas with sovereignty disputes); "
            "Direct Selection / Emergency Procurement (direct contracting when delays would cause "
            "harm — distinct from HEIS); "
            "Bank-Facilitated Procurement (BFP — Bank takes over procurement functions when borrower "
            "lacks capacity, stronger than HEIS advisory support); "
            "Deferred ESF Instruments (ESF allows deferral of some E&S instruments from preparation "
            "to early implementation, provided a timetable is agreed — significant FCV flexibility)."
        ),
        "not_applicable": (
        ...
        "cdd_sub_modality": (
            "Community-Driven Development (CDD) is a delivery approach used within IPF operations "
            "where communities directly control decisions and resources. CDD-style projects include "
            "those delivered through WASHCOs, water user associations, community health workers, "
            "village savings groups, and similar community management structures. "
            "FCV-specific risks of CDD delivery: (1) Elite capture — community management committees "
            "may be captured by locally powerful groups, excluding conflict-affected or marginalised "
            "populations; (2) Social exclusion — demand-driven CDD models systematically disadvantage "
            "communities with low organisational capacity (often the most conflict-affected); "
            "(3) Ethnic or clan bias — committee composition may reflect dominant ethnic groups, "
            "reinforcing exclusion of minorities, IDPs, or rival clans; (4) Accountability gaps — "
            "where state presence is weak, community oversight mechanisms may be insufficient to "
            "prevent resource diversion. "
            "When assessing a CDD-style IPF: verify whether the beneficiary selection methodology "
            "explicitly addresses how conflict-affected or socially marginalised communities will be "
            "included despite lower organisational capacity. A CDD model in a fragile context without "
            "explicit inclusion safeguards should be flagged under DNH Principles 1, 2, and 5."
        ),
        "non_state_actor_engagement": (
            "Para 18 of the World Bank Policy on Financing covers engagement where borrower "
            "governments have limited control of territory — relevant for operations in areas with "
            "armed groups, customary authorities, or fragmented sovereignty. "
            "Key implications: (1) The Bank may engage non-state actors as implementation partners "
            "where state delivery is not feasible, provided a political risk assessment is conducted; "
            "(2) Non-state actor engagement requires specific legal arrangements and Board disclosure; "
            "(3) In practice, engagement with UN agencies (UNICEF, UN-Habitat, WFP), international "
            "NGOs, and community structures often serves the Para 18 function without formal trigger. "
            "Assessment guidance: For projects operating in areas with contested state authority "
            "(e.g., Afar and Somali in Ethiopia, north-eastern Nigeria, eastern DRC, Sahel border "
            "zones), assess whether: (a) alternative implementation arrangements are considered; "
            "(b) the PAD acknowledges limits of government delivery systems; (c) UN or NGO "
            "partnerships are evaluated as a feasibility option. Absence of this consideration "
            "in high-insecurity areas is a material gap — the tool should flag it under "
            "Implementation Arrangements."
        ),
        "recent_changes": (
    ...
    "PforR": {
        ...
        "fcv_levers": (
            ...
            "Contingent DLIs — explicitly designed to be contingent on security conditions or "
            "context triggers, allowing disbursement flexibility (this is an FCV lever, not a "
            "design flaw — the tool should recognise these positively); "
    ...
    "DPO": {
        ...
        "fcv_levers": (
            ...
            "Deferred Drawdown Option (DPF-DDO / Cat-DDO) — funds committed but not drawn until "
            "a crisis trigger is met; the Catastrophe DDO (Cat-DDO) is specifically for natural "
            "disaster/crisis preparedness, relevant in FCV settings as contingency financing; "
            "PSIA with FCV lens — Poverty and Social Impact Analysis with explicit attention to "
            "conflict sensitivity, distributional effects across identity groups, and risk of exclusion."
        ),
        "not_applicable": (
...
DNH_SEASH_IPF = """SEA/SH risk management in IPF: assess project-specific exposure and the measures in the documents actually supplied. (a) Use a formal SEA/SH risk rating (Low / Moderate / Substantial / High) only when the project record or ESRS states it. Do not infer a SEA/SH rating from overall E&S risk. (b) Identify any planned, prepared or implemented GBV/SEA/SH Action Plan and its actual ESCP timing when the ESCP is supplied. An action plan incorporated into an ESIA or ESMP is not missing merely because it is not standalone. Do not infer a missed gate from a PAD that describes a design-stage plan. (c) Check whether the documented grievance mechanism can receive SEA/SH complaints safely and confidentially, with appropriate survivor referral, and distinguish project-specific ESCP commitments from optional design approaches. (d) Check applicable ESS2 worker provisions and monitoring against the supplied LMP, ESCP and project record; ESS4 covers security personnel and community safety where relevant. If an instrument is not supplied, request it rather than declaring its contents absent. Set seash_standalone_flag: TRUE only when a material SEA/SH risk or a specific inadequacy is supported by project evidence; explain the source, status and applicable requirement. The flag asks Stage 3 to consider a priority; it does not require a separate card or a standalone plan. Pass this flag to Stage 3."""

DNH_SEASH_PFORR = """SEA/SH risk management in conflict contexts (PforR) — for programs operating in conflict-affected areas, or involving contractor/implementing-partner workforces, or with female-majority beneficiaries or community workers, assess: (a) Does the ESSA (Environmental and Social Systems Assessment) evaluate the borrower's SEA/SH risk — drawing on the Program-for-Results core principles on public and worker safety and on protecting the poor and vulnerable, together with the Bank's Good Practice Note on SEA/SH — including whether the ESSA identifies and assesses this risk within its social risk analysis given the conflict context? (The distinct PforR core principle on avoiding the exacerbation of social conflict is FCV-relevant but is not the SEA/SH authority.) (b) Where the ESSA finds SEA/SH risk to be significant, does the Program Action Plan (PAP) include a time-bound commitment to strengthen SEA/SH prevention and response measures within the country's own systems? (c) Are GRM reporting channels — within the borrower's own systems as assessed in the ESSA, with gaps addressed through the PAP — anonymous/confidential and accessible to women with mobility restrictions? (d) Does the ESSA address contractor/implementing-partner screening for prior SEA/SH incidents where the program finances civil works or service delivery through non-state actors? (e) Is there a results/verification indicator tracking SEA/SH monitoring? PforR is not ESF-governed and has no environmental-and-social commitment plan, numbered environmental/social standards, or standalone SEA/SH Action Plan template, and does not apply the ESF's categorical SEA/SH risk classification (Low / Moderate / Substantial / High) — do not require these instruments or that rating construct; the ESSA's social risk analysis and the PAP are the governing mechanisms. Set seash_standalone_flag: TRUE where the ESSA identifies SEA/SH risk as significant, or if any of the five elements above is absent or inadequate. Pass this flag to Stage 3."""
...
SEASH_GENDER_CARD_IPF = """Gender-FCV priority rule for IPF: If gender_fcv_flag is TRUE and Stage 2 identifies a material project-specific gender or SEA/SH risk, consider it within the overall priority limit. ESS2 worker protections and ESS4 community safety should be assessed where applicable. The ESS1 risk assessment and SEA/SH Good Practice Note inform proportionate mitigation; the Good Practice Note is advisory, not a blanket project obligation. State the documented risk and existing or planned mitigation before proposing additional measures. Use the formal SEA/SH rating only if the project record or ESRS supplies it. Do not treat a missing standalone plan as a gap when an Action Plan is planned or incorporated into an ESIA or ESMP. Cite the relevant ESCP commitment, if supplied, and its actual timetable; otherwise ask the team to provide the ESCP and current plan status. Applicable document targets may include the SEP, LMP, ESMP or Operations Manual when those instruments are supplied or clearly planned. Grievance handling should preserve safe, confidential and survivor-centered receipt and referral, while channel design is proportionate to project risk and the actual commitments. If seash_standalone_flag is TRUE, consider a distinct SEA/SH priority only if the evidenced risk warrants it; do not duplicate a Gender-FCV or GRM card. Label good-practice options as recommendations rather than binding requirements. Recommend specialist review where the risk rating, plan status or grievance design remains unverified."""

SEASH_GENDER_CARD_PFORR = """Gender-FCV Card Rule (PforR): If gender_fcv_flag: TRUE was passed from Stage 2, a Gender-FCV priority card is mandatory, in addition to the standard 4-5 priorities. This card must address: how the FCV context specifically alters gender dynamics for this program, adequacy of the ESSA's SEA/SH risk assessment (under the Program-for-Results core principles on public and worker safety and on protecting the poor and vulnerable, together with the Bank's Good Practice Note on SEA/SH), GRM access for women and girls operating within the borrower's own systems, and safety risks for female community workers or service providers financed through the program. Document locations must name the relevant ESSA finding, PAP action, and POM (Program Operations Manual) provision. Do NOT reference the ESF commitment plan, numbered environmental/social standards, or a standalone SEA/SH Action Plan — these instruments do not exist for PforR. Recommend engagement with the Bank's SEA/SH Secretariat and Gender Group as the follow-up mechanism.
...
ESS4 paragraphs 24-27 apply when the Borrower retains direct or contracted security workers to safeguard project personnel or property, or when government security personnel are deployed to provide project security. Assess the risks posed by those arrangements to people within and outside the site; do not infer that security personnel are engaged merely because a PAD notes contextual criminal activity.

### Assessment and Proportionate Arrangements
Under ESS4 paragraph 24, direct or contracted security workers may use force only for preventive or defensive purposes proportional to the threat. The Borrower makes reasonable inquiries about past abuses by direct or contracted security workers, checks their training, and reviews allegations of unlawful or abusive acts. For government personnel, paragraph 25 calls on the Borrower to seek consistent conduct. A security risk assessment can be part of wider E&S assessment; its scope depends on project risks. The Good Practice Note offers methods and examples, not a universal document list.

### Security Management Plan
The Bank and Borrower agree whether a stand-alone SMP is required. The Good Practice Note says that lower-risk arrangements may instead be covered in the ESCP or ESMP, while a stand-alone plan can suit more complex or higher-risk cases. Check the project-specific ESCP before naming a required plan, its delivery date, or a compliance gap. If the source package contains only a PAD, ask for the ESCP and current security assessment rather than declaring a plan absent. Where an SMP is a documented project commitment, cite that instrument and timetable rather than attributing the obligation to the Good Practice Note alone.

### Management and Monitoring
Use project-specific documents and proportionate risk assessment to determine procedures for personnel conduct, training, grievance intake, incident notification, consultation, and review frequency. Preserve confidential handling of sensitive complaints. Do not invent an immediate Bank-alert deadline, a mission-by-mission SMP review, a mid-term audit, or a blanket prohibition on security personnel near consultations without an applicable source. Reassess arrangements when material risks or conditions change.""",
    },
    "rra_methodology": {
...
ESS1 (Assessment and Management of Environmental and Social Risks and Impacts) is the foundational standard and the most FCV-critical: it governs risk classification, the mitigation hierarchy, ESCP design, and differentiated measures for vulnerable groups. ESS4 (Community Health and Safety) is equally critical in FCV contexts because it covers security personnel conduct, community exposure to project-related conflict risk, and emergency preparedness. ESS10 (Stakeholder Engagement and Information Disclosure) drives GRM design and meaningful consultation requirements — both of which are structurally challenging when state access is limited or communities distrust government channels. ESS4 paragraphs 24-27 govern security personnel when the Borrower retains direct or contracted security workers or government personnel provide project security: assess risks from the arrangements, apply proportionality, make reasonable inquiries about past abuses, train personnel, and review allegations. ESS2 addresses labour and working conditions.
...
The Environmental and Social Commitment Plan (ESCP) is the legally binding instrument recording what the Borrower has committed to do, and by when. Check the project-specific ESCP for the E&S assessment, stakeholder engagement, differentiated measures, contractor management, and any security arrangements required by assessed risks or the use of security personnel. Do not infer a missing security plan from a PAD alone; obtain the ESCP and applicable ESIA/ESMP before calling an instrument absent or overdue. For PADs, draft ESCP must be disclosed before appraisal. FCV reviewers should check whether the ESCP includes realistic implementation timelines given conflict-reduced state capacity, and whether it names specific sub-national areas with distinct risk profiles.
...
ESS10 requires an accessible and inclusive project grievance mechanism, culturally appropriate, at no cost and without retribution. Check the actual ESCP and SEP for project-specific commitments on anonymity, response procedures and SEA/SH handling. In FCV settings, community-based, mobile, digital or third-party reception may improve safe access where formal channels are distrusted or inaccessible. A separate or third-party channel is a design option, not an automatic ESS10 requirement; assess whether the chosen mechanism works for affected groups and can handle sensitive complaints safely.

### FCV CC Reviewer Checklist — ESF Section of a PAD
First checks: (1) Is the SORT risk rating consistent with ESF-identified risks, particularly community conflict, security arrangements, and land/displacement risks? (2) Does the ESCP include a realistic timeline given Borrower capacity? (3) Does the SEP identify disadvantaged or vulnerable groups specifically (IDPs, host communities, women, ethnic minorities) and specify differentiated engagement methods for each? (4) Can affected groups use the documented grievance channels safely and confidentially? (5) If security personnel are planned, are the ESS4 paragraphs 24-27 risks and conduct standards addressed? Common gaps: SEP relies on government-led consultations without alternative channels; GRM is a phone number at a ministry; ESCP timelines assume pre-conflict state institutional capacity.
""",
    },
