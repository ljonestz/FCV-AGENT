...
**Version history:**
- **v9.31** - Climate-FCV Stage 3 concise presentation (2026-08-13): verified Climate-FCV design reviews now use the existing accessible Summary/Detailed tabs without another model request. Summary is a deterministic projection of the canonical reader: a short executive excerpt, a compact text sensitivity signal, dynamically selected evidence-backed existing responses with positive judgment fallbacks, and the ranked reader priorities. The existing verified reader remains the Detailed view, including its segmented sensitivity rating. Core FCV Summary gating remains unchanged. The concise lifecycle card now uses an explicit label/value grid so Before Appraisal and At Design Stage align in one value column. `build_reader_model()` exposes up to 12 safe existing-response records for the browser projection. Tests cover route gating, dynamic strengths, rating-bar separation, reader data, and lifecycle alignment.
...
**Verified Climate-FCV Stage 3 presentation:** The v2.1 reader now has a deterministic browser Summary/Detailed toggle. Summary uses a short executive excerpt, compact text sensitivity signal, variable strength cards from safe existing-response records with positive judgment fallbacks, and ranked reader priorities; the existing verified reader, including its segmented sensitivity rating, remains Detailed. This adds no model request and does not widen the core FCV Summary gate.
