...
- **v9.36** - Deterministic Climate project-profile selection (same branch, 2026-08-11). Climate-active step-by-step and Express preparation now build one bounded `ProjectClimateProfile` from explicit controlled-vocabulary matches in project-document text and existing document metadata, then use that same typed profile for balanced country-bank selection. Candidate-bank signals remain labelled candidate/unresolved unless the document explicitly matches them. Safe selection diagnostics retain only canonical IDs, integer scores, allowlisted matched fields, controlled balance/staleness/suppression values, and missing evidence classes. Server-side rematerialization preserves the full canonical source records for provenance while prompt grounding emits separate rich evidence and pathway capsules and drops whole lowest-priority capsules to keep the existing 6,000-character bank and 12,000-character combined ceilings. Schema 1.0 records remain compatible through conservative analytical-role mapping; candidate preview remains visibly non-approved. Live research still receives its existing bounded project input, uses no additional search or model call, and remains non-fatal. No prompt, rating, recommendation, reader hierarchy, generic-FCV route, reviewed companion-bank content, or Render configuration changed. Deferred increments remain literature/content expansion, release promotion, additional countries, multi-country allocation, and gap-directed live-research questions/telemetry.
