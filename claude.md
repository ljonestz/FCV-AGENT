...
- **v9.24** - Verified Climate-FCV evidence-to-decision runtime (branch `feat/climate-country-bank`, 2026-08-01):
  ...
  - `climate_assessment` and the canonical `climate_reader` are additive SSE fields. Live HTML and shared HTML render the server reader directly; DOCX deterministically rebuilds the same reader from the assessment. Reader state is preserved in session files and completed Express checkpoints; candidate-bank outputs retain `preview; not approved`.
  ...
  - Privacy-safe recommendation diagnostics record raw, parsed, structurally valid, gate-admitted, and final counts; deterministic admission reason codes; and whether the conditional semantic reviewer ran and its bounded verdict. The same fields appear in the technical annex and one bounded Render log line, without candidate prose.
...
**Current version:** FCV Project Screener v9.24
